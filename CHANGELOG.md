...
- Added Producer/Supervisor Agent 0.1.1 with idempotent dispatch, evidence supervision, real progress, recovery, failed-only retry, credit aggregation, NDJSON/HTTP protocols, and fail-closed human authorization boundaries.
- Added a partial semantic Pipeline adapter for seven deterministic generic production gates; provider-backed media generation remains explicitly `ADAPTER_REQUIRED`.
...
- Upgraded Story Agent to 0.2.0 with fast US premium-streaming pacing prompts and deterministic opening-hook, end-hook, repeated-dialogue, dialogue-density, and non-advancing-shot checks.
