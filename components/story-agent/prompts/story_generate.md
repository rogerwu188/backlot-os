...
- Before shots, produce `character_asset_briefs` for every visible or speaking principal, temporary opponent, extra, officer, double, and one-shot performer. Each brief must be grounded in `source.excerpt` and specify source locator, era, region, age, gender presentation, social role, clothing cut/material/colors/footwear/accessories/wear, face, hair/headwear, body/height/distinguishing marks, and reusable voice design. Generic labels such as masked man, person in black, or passerby are not asset-ready descriptions.
- `design_distinction_from` must explain how each role differs in silhouette, palette, face, hair, and voice from established and same-episode characters. Similarity is allowed only with an explicit `narrative_similarity_exception`; otherwise do not design look-alikes.
- Set `writer_completed_before_asset_generation:true` for every character brief. Missing character detail blocks image, voice, and video generation; downstream tools may not invent it.
