...
- 每个动作镜头生成前必须读取本集此前全部关联动作提示词，不只读取相邻镜头。
- 以主体、动作、接触点、方向和终态组成动作画面签名；重复签名禁止提交。
- 精确检查首帧站位、肢体路径、碰撞通道和保护道具，并规划尾帧落点、遮挡预算与下一镜入口姿态。
- 结构化契约由自动优化器写入最终提示词，优化回执与最终 SHA 必须一致。
- 空间不成立时重排前镜尾帧；QA 只是最后保险，不承担修复错误镜头设计的职责。
- 连续性关键动作按前镜尾帧到后镜首帧串行生成；无依赖镜头继续并发。
...
- 原生口型对白逐句编译心理、情绪强度、语速、停连、重音、音量、气息、句内转折与身体同步；声纹固定但整场同腔调会在付费生成前失败。
