...
- 打斗 prompt 必须把镜头语言绑定到具体动作拍、时间、主体锚点、轴线和叙事目的；未声明时段默认固定机位。15 秒长镜最多两个动态运镜段，中间至少 1 秒稳定观察；慢动作仅限唯一决定性接触且不超过 0.6 秒，冲击震动不超过 0.35 秒。禁止把跟拍、环绕、推拉、升降、甩镜或旋转连续堆叠成装饰性摇镜。
...
- QA 必须并行调度：候选素材落盘后，视觉、对白完整/重复、OCR、角色与声音一致性、动作空间与胜负终态、节奏和精确积分核对立即同时启动；不同已完成镜头之间也并行。仅最终接纳门等待全部必检结果汇总。单项失败不得取消其余 QA，否则会丢失同轮问题证据并拖慢返修。
- 只有存在生成依赖的动作链保持串行，例如“前镜尾帧=后镜首帧”；该串行约束不得扩散到同一镜头的独立 QA，也不得阻塞其他无依赖镜头的 QA。
...
Seedance prompt 不写否定敏感词。避免 `无血腥`、`无武器`、`无冲突`、`不要英文`、`不要欧美人物` 这类表达；改写为正向描述，例如 `平静克制`、`普通文件`、`华语医院短剧质感`、`纸张轻响`、`冷蓝灯光`。如为过审暂时去掉对白，该镜头只能标记为 `Audio Rework Required` 的内部素材；正式发行前必须用 Seedance 2 多模态或统一 `VOICE-ID` 补齐中文对白、声桥和音效，并对最终成片做 ASR/VAD。
...
- 本地统一重配音时，原平台音轨必须全量静音；禁止低音量保留原声再叠加新对白。
- 抽帧没有欧美人物混入。
- 每个说话镜头的台词听起来完整。
- 每个复现角色都与角色库和上一集抽帧一致。
