...
class EpisodeParallelBatchSupervisorActivityTest(unittest.TestCase):
    ...
    def test_exact_provider_captions_are_allowed_with_bound_review_contract(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            image = root / "anchor.png"
            audio = root / "D1.wav"
            ...
            failures = validate_entity_reference_task({
                ...
                "reference_audios": [str(audio)],
                "native_dialogue_required": True,
                "dialogue_visual_text_policy": "EXACT_PROVIDER_CAPTION_ALLOWED",
                "dialogue_visual_text_contract": {
                    "exact_texts": ["别动。"],
                    "source_binding_sha256": "a" * 64,
                    "style_brief": "white outlined period-drama subtitle",
                    "final_subtitle_strategy": "PROVIDER_CAPTIONS_ONLY",
                    "ocr_review_required": True,
                    "human_review_required": True,
                    "duplicate_agentcut_subtitles_forbidden": True,
                },
                "dialogue": [{"dia_id": "D1", "speaker": "刺客", "spoken_text": "别动。"}],
                "dialogue_audio_assets": [{
                    "dia_id": "D1", "path": str(audio),
        ...
        checks = {row["check"] for row in failures}
        self.assertNotIn("dialogue_visual_text_isolation", checks)
        self.assertNotIn("dialogue_visual_text_contract", checks)
        self.assertNotIn("dialogue_visual_text_review", checks)
        self.assertNotIn("dialogue_visual_text_exact_caption_coverage", checks)

    def test_exact_provider_captions_fail_without_exact_coverage_and_review(self):
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            image = root / "anchor.png"
            audio = root / "D1.wav"
        ...
        checks = {row["check"] for row in failures}
        self.assertIn("dialogue_visual_text_review", checks)
        self.assertIn("dialogue_visual_text_exact_caption_coverage", checks)
