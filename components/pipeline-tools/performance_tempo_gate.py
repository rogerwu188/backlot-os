...
def _prompt_text(task: dict[str, Any]) -> str:
    parts = [
        task.get("shot_purpose"),
        task.get("narrative_function"),
        task.get("prompt"),
...
def _looks_like_action(task: dict[str, Any], text: str) -> bool:
    if task.get("action_unit") is True:
        return True
    if any(task.get(name) for name in (
    ...
    *,
    ...
    for index, window in enumerate(windows, 1):
        ...
        if start - previous_end > MAX_ACTION_IDLE_GAP_SECONDS:
            failures.append({
                "code": "ACTION_IDLE_GAP_INVITES_SLOW_MOTION",
                "task_key": key,
                "index": index,
                "gap_seconds": round(start - previous_end, 3),
            })
        span = end - start
        if span > maximum + 0.01:
            failures.append({
                "code": "ATOMIC_ACTION_WINDOW_TOO_LONG",
                "task_key": key,
                "index": index,
                "action": window.get("action"),
                "actual_seconds": round(span, 3),
    ...
    if fight_or_chase and first_start is not None and first_start > MAX_FIGHT_ONSET_SECONDS:
        failures.append({
            "code": "FIGHT_ACTION_ONSET_TOO_LATE",
            "task_key": key,
            "actual_seconds": first_start,
            "maximum_seconds": MAX_FIGHT_ONSET_SECONDS,
        })
    return failures
