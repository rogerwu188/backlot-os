...
| `backlotos-pipeline-command` (BACKLOT_PIPELINE_COMMAND) | `health`, `gate`, `parallelQa`, `edit-plan-integrity`, `providerHealth`, `generateImage`, `generateVideo`, `taskStatus` | SUPPORTED; live generation is ADAPTER_REQUIRED until configured | Generic gates plus a Giggle provider. `parallelQa` concurrently fans out independent per-shot and cross-shot QA, isolates failures, preserves deterministic result order, and aggregates only after all required checks finish. Credential is environment-only. Defaults are `gpt2img` and standard-only `seedance-2.0`; Pro, fast, and mini fail before transport. |
